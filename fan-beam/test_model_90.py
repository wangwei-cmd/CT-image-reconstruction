...
# pre='./weights_37.87_37.93'
pre='./512x512/fan-beam/weights'
ckpt=[pre+'/red_cnn',pre+'/DD-NET',pre+'/fbpconv',
...
Model=[net0.make_model(batch),net1.make_model(batch),net2.make_model(batch),
       net3.make_model_3(AT, alpha, w_c, s_shape, out_size)
       ]


# data=np.load('./512x512/fan-beam/'+'train'+'_fan_data.npz')
...
prediction=evaluate(j,f_noisy_img,vx,batch,shape,Model)
...
plt.show()
pp=tf.image.psnr(tf.cast(prediction,tf.float32),vy,tf.reduce_max(prediction))
pp1=tf.image.psnr(vx,vy,tf.reduce_max(vx))
qq=tf.image.ssim(tf.cast(prediction,tf.float32),vy,tf.reduce_max(prediction))
qq1=tf.image.ssim(vx,vy,tf.reduce_max(vy))
