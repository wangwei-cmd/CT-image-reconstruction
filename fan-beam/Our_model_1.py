...
class sinLayer(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        for i in range(1,len(self.sinLayer)):
            de_sin=self.sinLayer[i](de_sin)
        de_sin=inputs+de_sin

        fbp=self.decode(de_sin)
        # fbp=decode(de_sin, self.AT, self.alpha, self.h, self.w_c)

        outputs = self.ctLayer[0](fbp)
        for i in range(1, len(self.ctLayer)):
            outputs = self.ctLayer[i](outputs)
        outputs=fbp+outputs
        # outputs=outputs*M/255
        # outputs = ctdncnn(fbp)
...
if __name__ == '__main__':
    ...
    udir = "./train/"
    vdir = "validate"
    train(epoch, udir, batch, theta, iternum, restore=0, ckpt='./512x512/fan-beam/weights/our_model_1_lambda=0.5')