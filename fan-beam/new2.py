...
class sinLayer(tf.keras.Model):
    def __init__(self, AT,alpha,w_c,s_shape = (360,1601),out_size=(512,512)):
        super(sinLayer, self).__init__()
        self.AT = AT
        self.w_c=w_c
        self.alpha=alpha
        self.h=alpha[1]-alpha[0]
        ...
        self.sinLayer_1.append(tf.keras.layers.Conv2D(64, 5, padding='same', name='sin1conv1', activation=tf.nn.relu))
        ###CTLayer###
        ...
        self.ctLayer_1.append(tf.keras.layers.Conv2D(64, 5, padding='same', name='ct1conv1', activation=tf.nn.relu))
...
@tf.function
def decode(sin_fan,AT,alpha,h,w_c):
    cos_alpha=tf.math.cos(alpha)
    ...
    rf = tf.sparse.sparse_dense_matmul(AT, filter_s_fan2)
    rf = tf.transpose(rf)
    rf = tf.reshape(rf, [s_fan_shape[0], 512, 512, 1])
    return rf


def make_model_3(AT,alpha,w_c,s_shape=(720, 1601),out_size=(512,512)):
    CT=sinLayer(AT,alpha,w_c,s_shape,out_size)
...
def train(epoch, udir,batch, theta, iternum, restore=0, ckpt='./weights/CT_tf2_4'):
    #####do not use tf.cast in train function since it will exhaust/use GPU memory
    s_shape = (360, 1601)
    out_size = (512, 512)

    data = np.load('train' + '_fan_data.npz')
    u_img=data['u'].astype('float32')
    f_img=data['sin_fan'].astype('float32')
    f_noisy_img=data['sin_fan_ini'].astype('float32')
    # u_ini=data['u_ini'].astype('float32')
    del data
    ...
    # alpha_1=np.linspace(-max_alpha, max_alpha, n2) * np.pi / 180
    # f_noisy_img = interp(f_noisy_img, alpha_1, alpha)
    # np.savez('./512x512/fan-beam/f_noisy_interp',f_noisy_img=f_noisy_img)
    # f_noisy_img =np.load('./512x512/fan-beam/f_noisy_interp.npz')['f_noisy_img']
    ...
    Model = make_model_3(AT, alpha, w_c, s_shape, out_size)
    # Model=sinLayer(AT,alpha,w_c,s_shape,out_size)
...
if __name__ == '__main__':
    ...
    udir = "./train/"
    vdir = "validate"
    train(epoch, udir, batch, theta, iternum, restore=0, ckpt='./512x512/fan-beam/weights/new1_model_lambda=0.5')