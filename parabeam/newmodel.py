...
class sinLayer(tf.keras.Model):
    ...
    #     for i in range(M1+1, self.M1+1):
    #         for j in range(0, 3):
    #             de_sin = self.sinLayer[3 * i + j - 2](de_sin)
    #         de_sin=de_sin+pp[i-M1-1]
    #     de_sin=self.sinLayer[3*self.M1+1](de_sin)+inputs
    #
    ...
    #     for i in range(M2+1, self.M2+1):
    #         for j in range(0, 3):
    #             outputs = self.ctLayer[3 * i + j - 2](outputs)
    #         outputs=outputs+qq[i-M2-1]
    #     outputs=self.ctLayer[3 * self.M2+1](outputs)+fbp
...
def train(epoch, udir,batch, theta, iternum, restore=0, ckpt='./weights/CT_tf2_4'):
    ...
    f_noisy_img = np.load(udir + 'f_noisy,angle='+str(angles)+'_no_scale__0.5.npy').astype('float32')
    f_img = np.load(udir + '/f,angle=' + str(180) + '_no_scale__0.5.npy').astype('float32')
    u_ini = np.load(udir + 'ini,angle='+str(angles)+'_no_scale__0.5.npy').astype('float32')
    ...
    f_noisy_img = interp(f_noisy_img, np.linspace(0, 180, angles, endpoint=False), np.linspace(0, 180, 180, endpoint=False))
...
if __name__ == '__main__':
    ...
    train(epoch, udir, batch, theta, iternum, restore=0, ckpt='./512x512/weights/new_model_lambda=0.5')